...
document.querySelector('aside .sticky').style.maxWidth = document.querySelector('aside').offsetWidth + 'px';

// HANDLE NAVBAR & STICKY SIDEBAR WHEN SCROLL
window.addEventListener('scroll', () => {
    let scrollPos = window.scrollY;
    let mainContainerPos = document.querySelector('.main-container').getBoundingClientRect().top;
    let invitationPos = document.querySelector('.invitation').getBoundingClientRect().top;


    console.log(mainContainerPos);
    scrollPos > 120 ? document.querySelector('header').classList.add('bg-primary-blue-1') : document.querySelector('header').classList.remove('bg-primary-blue-1')
